...
module.exports.bubble = (game, dir, pos, text) => {
	if (text.charAt(text.length - 1) === '\n'){
				text = text.substring(0, text.length - 1);
			}
return 'https://nfggames.com/system/arcade/arcade.php/b-' + dir + '/bp-' + pos + '/y-' + game + '/z-0/dbl-2/x-' + encodeURI(text  + '\u200B');
}

module.exports.SSbubble = (game, dir, pos, style, size, text) => {
	if (text.charAt(text.length - 1) === '\n'){
				text = text.substring(0, text.length - 1);
			}
